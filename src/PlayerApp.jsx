...
export default function PlayerApp() {
  ...
  const refresh=async()=>{
    const [songs,favs,recent,pls,pfs,dirs,savedQueue,savedSettings,libraryStats,searches,cache]=await Promise.all([
      api.library(),api.favorites(),api.historyList(),api.playlists(),api.playlistFolders(),api.folders(),api.queue(),api.settings(),api.stats(),api.recentSearches(),api.cacheStatus()
    ])
    setLibrary(songs);setFavorites(favs);setHistory(recent);setPlaylists(pls);setPlaylistFolders(pfs);setFolders(dirs);setQueue(savedQueue);setSettings(savedSettings);setStats(libraryStats);setRecentSearches(searches);setCacheInfo(cache)
  }
  ...
  const pageContent=()=>{
    ...
    if(page==='settings')return <SettingsPage t={t} lang={lang} settings={settings} saveSettings={saveSettings} folders={folders} chooseFolder={chooseFolder} scanFolders={scanFolders} scanSystem={scanSystem} busy={busy} stats={stats} cacheInfo={cacheInfo} clearCache={clearCache} exportBackup={exportBackup} importBackup={importBackup}/>
    return null
  }
  ...
  return <div className={`app-shell ${settings.reduceMotion?'reduce-motion':''}`}>
    ...
    {settingsOpen&&<Modal title={t('settings')} onClose={()=>setSettingsOpen(false)} wide><SettingsPage compact t={t} lang={lang} settings={settings} saveSettings={saveSettings} folders={folders} chooseFolder={chooseFolder} scanFolders={scanFolders} scanSystem={scanSystem} busy={busy} stats={stats} cacheInfo={cacheInfo} clearCache={clearCache} exportBackup={exportBackup} importBackup={importBackup}/></Modal>}
...
function SettingsPage({t,lang,settings,saveSettings,folders,chooseFolder,scanFolders,scanSystem,busy,stats,cacheInfo,clearCache,exportBackup,importBackup,compact=false}){

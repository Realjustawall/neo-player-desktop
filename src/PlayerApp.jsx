...
import {
  Album, AudioLines, BarChart3, ChevronDown, ChevronLeft, ChevronRight, ChevronUp, Clock3, Disc3, Download,
...
import { api } from './api'
import { ACCENTS, FONT_OPTIONS, THEME_PRESETS, applyAppearance } from './theme'
import { translator } from './i18n'
import packageInfo from '../package.json'

const FORMAT_OPTIONS=['.mp3','.wav','.flac','.m4a','.aac','.ogg','.opus','.wma','.aiff','.aif','.ape','.webm']
...
const normalizeSearch = (value='') => String(value).normalize('NFKD').replace(/[\u064B-\u065F\u0670\u06D6-\u06ED]/g,'').replace(/[يى]/g,'ی').replace(/ك/g,'ک').replace(/[\u200c\u200d]/g,' ').toLocaleLowerCase().trim()

function NeoLogo({ size=38 }) {
  return <img className="neo-logo" src="/neo-logo.png" width={size} height={size} alt="NEO PLAYER" draggable="false" />
}
...
export default function PlayerApp() {
  ...
  const masterGainRef=useRef(null), widthRef=useRef({}), intentPlaying=useRef(false), recoveryAt=useRef(0)
  ...
  const [history,setHistory]=useState([]), [folders,setFolders]=useState([]), [playlists,setPlaylists]=useState([]), [playlistFolders,setPlaylistFolders]=useState([]), [pins,setPins]=useState([]), [hiddenSongs,setHiddenSongs]=useState([]), [stats,setStats]=useState({})
  ...
  const [scopeProfile,setScopeProfile]=useState(null), [scopeEditor,setScopeEditor]=useState(null)
  ...
  const [recentSearches,setRecentSearches]=useState([]), [playlistQuery,setPlaylistQuery]=useState(''), [cacheInfo,setCacheInfo]=useState({files:0,bytes:0})
  const [installedFonts,setInstalledFonts]=useState([]), resumeDone=useRef(false)
  ...
  const refresh=async()=>{
    const [songs,allSongs,favs,recent,pls,pfs,savedPins,dirs,savedQueue,savedSettings,libraryStats,searches,cache]=await Promise.all([
      api.library(),api.library('',true),api.favorites(),api.historyList(),api.playlists(),api.playlistFolders(),api.pins(),api.folders(),api.queue(),api.settings(),api.stats(),api.recentSearches(),api.cacheStatus()
    ])
    setLibrary(songs);setHiddenSongs(allSongs.filter(x=>x.hidden));setFavorites(favs);setHistory(recent);setPlaylists(pls);setPlaylistFolders(pfs);setPins(savedPins);setFolders(dirs);setQueue(savedQueue);setSettings(savedSettings);setStats(libraryStats);setRecentSearches(searches);setCacheInfo(cache)
  }
  useEffect(()=>{refresh().catch(e=>showToast(e.message))},[])
  useEffect(()=>{(async()=>{try{const x=await window.pywebview?.api?.installed_fonts?.();setInstalledFonts(Array.isArray(x)?x:[])}catch{}})()},[])
  useEffect(()=>{applyAppearance(settings,profile,lang)},[settings.themeMode,settings.themePreset,settings.interfaceDensity,settings.accent,settings.customColor,settings.fontFamily,settings.fontScale,lang,profile?.accent])
  ...
  const ensureGraph=async()=>{
    if(!audioContext.current){
      ...
      sourceA.connect(ga);sourceB.connect(gb);ga.connect(fs[0]);gb.connect(fs[0]);for(let i=0;i<fs.length-1;i++)fs[i].connect(fs[i+1])
      const splitter=ctx.createChannelSplitter(2),merger=ctx.createChannelMerger(2),ld=ctx.createGain(),rd=ctx.createGain(),lc=ctx.createGain(),rc=ctx.createGain(),master=ctx.createGain()
      ld.gain.value=1;rd.gain.value=1;lc.gain.value=0;rc.gain.value=0;widthRef.current={ld,rd,lc,rc};masterGainRef.current=master
      fs.at(-1).connect(splitter);splitter.connect(ld,0);ld.connect(merger,0,0);splitter.connect(rd,1);rd.connect(merger,0,1);splitter.connect(lc,0);lc.connect(merger,0,1);splitter.connect(rc,1);rc.connect(merger,0,0);merger.connect(master);master.connect(analyser);analyser.connect(ctx.destination)
    }
    if(audioContext.current.state==='suspended')await audioContext.current.resume()
  }
  useEffect(()=>{const override=profile?.audioEnabled?profile:scopeProfile?.enabled?scopeProfile:null;const eq=override?.eq||settings.eqBands||[];const enabled=override?true:settings.eqEnabled;filtersRef.current.forEach((f,i)=>{f.gain.value=enabled?Number(eq[i]||0):0});if(filtersRef.current[0])filtersRef.current[0].gain.value+=(Number(override?.bass??settings.bassBoost??0)/100)*8;const width=Number(override?.virtualizer||0)/100;if(widthRef.current.ld){widthRef.current.ld.gain.value=1+width*.25;widthRef.current.rd.gain.value=1+width*.25;widthRef.current.lc.gain.value=-width*.16;widthRef.current.rc.gain.value=-width*.16}const loudness=override?Number(override.loudness||0):(settings.loudnessNormalization?Number(settings.normalizationTargetLufs||-14)+14:0);if(masterGainRef.current)masterGainRef.current.gain.value=Math.pow(10,loudness/20)},[profile?.audioEnabled,JSON.stringify(profile?.eq),profile?.bass,profile?.virtualizer,profile?.loudness,JSON.stringify(scopeProfile),settings.eqEnabled,JSON.stringify(settings.eqBands),settings.bassBoost,settings.loudnessNormalization,settings.normalizationTargetLufs])
  ...
  const playTrack=async(song,context=library,crossfade=false,audioProfile=null)=>{
    ...
    try{
      ...
      intentPlaying.current=true;setScopeProfile(audioProfile);setCurrent(song);setPlaying(true);setPosition(0);setDuration(song.duration||0);api.history(song.id).catch(()=>{})
    }catch(e){intentPlaying.current=false;setPlaying(false);showToast(e.message)}
  }
  const nextTrack=(fromEnded=false,withCrossfade=true)=>{
    const list=queue.length?queue:library;if(!list.length)return
    if(settings.repeat==='one'&&fromEnded){const a=getDeck();intentPlaying.current=true;a.currentTime=0;a.play().catch(()=>{intentPlaying.current=false});return}
    ...
    if(idx>=list.length){if(settings.repeat==='all')idx=0;else{intentPlaying.current=false;setPlaying(false);return}}
    playTrack(list[idx],list,withCrossfade,scopeProfile)
  }
  const prevTrack=()=>{const a=getDeck();if(a&&a.currentTime>4){a.currentTime=0;return}const list=queue.length?queue:library;if(!list.length)return;let idx=list.findIndex(x=>x.id===current?.id)-1;if(idx<0)idx=settings.repeat==='all'?list.length-1:0;playTrack(list[idx],list,true,scopeProfile)}
  const togglePlay=async()=>{const a=getDeck();if(!current&&library.length)return playTrack(library[0],library);if(!a)return;await ensureGraph();if(a.paused){intentPlaying.current=true;a.play().catch(()=>{intentPlaying.current=false})}else{intentPlaying.current=false;a.pause()}}
  ...
  useEffect(()=>{
    const stopIntent=()=>{intentPlaying.current=false}
    const startIntent=()=>{intentPlaying.current=true}
    const recover=()=>{if(!settings.resumeOnAudioDeviceChange||!current||!intentPlaying.current)return;const now=Date.now();if(now-recoveryAt.current<1200)return;recoveryAt.current=now;setTimeout(async()=>{const audio=getDeck();if(!audio||!intentPlaying.current)return;try{await ensureGraph();if(audioContext.current?.state==='suspended')await audioContext.current.resume();if(audio.paused){const at=audio.currentTime||0;audio.load();await new Promise((resolve,reject)=>{const ok=()=>{cleanup();resolve()};const bad=()=>{cleanup();reject(new Error('output recovery failed'))};const cleanup=()=>{audio.removeEventListener('canplay',ok);audio.removeEventListener('error',bad)};audio.addEventListener('canplay',ok,{once:true});audio.addEventListener('error',bad,{once:true});setTimeout(ok,2500)});if(Number.isFinite(at)&&at>0)try{audio.currentTime=at}catch{};await audio.play();setPlaying(true)}}catch{}},300)}
    let outputSignature=''
    const checkOutputs=async()=>{try{const devices=await navigator.mediaDevices?.enumerateDevices?.();const next=(devices||[]).filter(d=>d.kind==='audiooutput').map(d=>`${d.deviceId}:${d.groupId}`).sort().join('|');if(outputSignature&&next&&next!==outputSignature)recover();if(next)outputSignature=next}catch{}}
    checkOutputs();const outputPoll=setInterval(checkOutputs,2500)
    navigator.mediaDevices?.addEventListener?.('devicechange',recover)
    window.addEventListener('neo:pause-intent',stopIntent);window.addEventListener('neo:play-intent',startIntent)
    return()=>{clearInterval(outputPoll);navigator.mediaDevices?.removeEventListener?.('devicechange',recover);window.removeEventListener('neo:pause-intent',stopIntent);window.removeEventListener('neo:play-intent',startIntent)}
  },[current?.id,settings.resumeOnAudioDeviceChange])
  useEffect(()=>{if(resumeDone.current||!library.length||!settings.resumeLastSong||!settings.lastSongId)return;const song=library.find(x=>x.id===Number(settings.lastSongId));resumeDone.current=true;if(!song)return;const audio=getDeck();audio.src=`/media/${song.id}`;audio.currentTime=Number(settings.lastPosition||0);setCurrent(song);setPosition(Number(settings.lastPosition||0));setDuration(song.duration||0)},[library,settings.resumeLastSong,settings.lastSongId])
  useEffect(()=>{if(!current||!settings.resumeLastSong)return;const timer=setInterval(()=>{const audio=getDeck();if(audio)api.patchSettings({lastSongId:current.id,lastPosition:audio.currentTime||0}).catch(()=>{})},5000);return()=>clearInterval(timer)},[current?.id,settings.resumeLastSong])
  ...
  const searchIndex=useMemo(()=>new Map(library.map(s=>[s.id,normalizeSearch([s.title,s.artist,s.album,s.genre,s.mood,s.key,s.year].join(' '))])),[library])
  const filtered=useMemo(()=>{const q=normalizeSearch(query);if(!q)return sortedLibrary;const terms=q.split(/\s+/).filter(Boolean);return sortedLibrary.filter(s=>terms.every(term=>(searchIndex.get(s.id)||'').includes(term)))},[sortedLibrary,query,searchIndex])
  ...
  const hideSong=async(song)=>{await api.hideSong(song.id,true);if(current?.id===song.id){intentPlaying.current=false;getDeck()?.pause();setPlaying(false);setCurrent(null)}await refresh();showToast(t('hide'))}
  const unhideSong=async(song)=>{await api.hideSong(song.id,false);await refresh();showToast(t('unhide'))}
  const isPinned=(kind,key)=>pins.some(p=>p.kind===kind&&p.itemKey===key)
  const togglePin=async(kind,key)=>{await api.setPin(kind,key,!isPinned(kind,key));setPins(await api.pins())}
  ...
  const moveQueue=async(i,delta)=>{const next=[...queue],target=i+delta;if(target<0||target>=next.length)return;[next[i],next[target]]=[next[target],next[i]];await persistQueue(next)}
  const rememberSearch=async(value=query)=>{const clean=value.trim();if(clean.length<2)return;setRecentSearches(await api.addRecentSearch(clean))}
  ...
  const chooseExcludedFolder=async()=>{let path=null;try{path=await window.pywebview?.api?.pick_folder?.()}catch{};if(!path)return;const values=settings.excludedFolders||[];if(!values.includes(path))await saveSettings({excludedFolders:[...values,path]})}
  const removeExcludedFolder=path=>saveSettings({excludedFolders:(settings.excludedFolders||[]).filter(x=>x!==path)})
  ...
  const addStream=async()=>{if(settings.strictOfflineMode){showToast(lang==='fa'?'ابتدا حالت کاملاً آفلاین را خاموش کن':'Disable Strict Offline mode first');return}const url=window.prompt(lang==='fa'?'لینک مستقیم HTTP/HTTPS آهنگ یا رادیو':'Direct HTTP/HTTPS audio or radio URL');if(!url?.trim())return;const title=window.prompt(lang==='fa'?'نام استریم (اختیاری)':'Stream title (optional)')||'';try{const song=await api.addStream(url.trim(),title);await refresh();showToast(`${t('success')} · ${song.title}`)}catch(e){showToast(e.message)}}
  ...
  const saveScopeProfile=async(data)=>{if(!scopeEditor)return;if(scopeEditor.kind==='playlist'){await api.patchPlaylist(scopeEditor.id,{audioProfile:data});if(selectedPlaylist?.id===scopeEditor.id)setSelectedPlaylist(await api.playlist(scopeEditor.id))}else{await api.patchPlaylistFolder(scopeEditor.id,{audioProfile:data})}await refresh();setScopeEditor(null);showToast(t('success'))}
  ...
  const clearCache=async()=>{const result=await api.clearCache();setCacheInfo(result);showToast(lang==='fa'?`${result.removed} فایل موقت پاک شد`:`${result.removed} temporary files cleared`)}
  const reorderPlaylistSong=async(index,delta)=>{if(!selectedPlaylist||selectedPlaylist.sortMode!=='custom')return;const songs=[...selectedPlaylist.songs],target=index+delta;if(target<0||target>=songs.length)return;[songs[index],songs[target]]=[songs[target],songs[index]];setSelectedPlaylist({...selectedPlaylist,songs});await api.reorderPlaylist(selectedPlaylist.id,songs.map(s=>s.id))}

  const renderTracks=(songs,extraBuilder,audioProfile=null)=> <div className="track-table"><div className="track-head"><span>#</span><span>{t('title')}</span><span>{t('album')}</span><span></span><Clock3 size={15}/></div>{songs.map((song,i)=><TrackRow key={`${song.id}-${i}`} song={song} index={i} current={current?.id===song.id} playing={playing} onPlay={s=>playTrack(s,songs,false,audioProfile)} onFavorite={toggleFavorite} onQueue={addQueue} onPlaylist={setPlaylistPickerSong} onHide={hideSong} extra={extraBuilder?.(song)}/>)}</div>
  ...
  const pageContent=()=>{
    if(!library.length && !['settings','create','hidden'].includes(page))return <EmptyState icon={<ScanSearch size={34}/>} title={t('noMusic')} hint={t('noMusicHint')} action={<div className="empty-actions"><button className="primary" onClick={chooseFolder}>{t('addFolder')}</button><button onClick={scanSystem}>{t('scanSystem')}</button></div>}/>
    if(page==='home')return <><section className="hero"><div><span className="eyebrow">NEO PLAYER · LOCAL FIRST</span><h1>{lang==='fa'?'موسیقی خودت، هوشمندتر و کاملاً محلی':'Your music, smarter and fully local'}</h1><p>{lang==='fa'?'پخش آفلاین، میکس هوشمند، متن هماهنگ، تم‌های کامل و کنترل عمیق روی کتابخانه.':'Offline playback, smart mixes, synced lyrics, rich themes and deep control over your library.'}</p><div className="hero-actions"><button className="primary" onClick={()=>smart()}><Sparkles size={18}/>{t('smartMix')}</button><button onClick={chooseFolder}><FolderPlus size={18}/>{t('addFolder')}</button></div></div><NeoLogo size={170}/></section>
      {!!pins.length&&<><SectionTitle title={t('pinned')}/><div className="entity-grid compact">{pins.map(p=>{const entity=p.kind==='album'?albums.find(x=>x.name===p.itemKey):p.kind==='artist'?artists.find(x=>x.name===p.itemKey):null;if(!entity)return null;return <EntityCard key={`${p.kind}-${p.itemKey}`} title={p.itemKey} subtitle={p.kind} song={entity.song} round={p.kind==='artist'} onClick={()=>{if(p.kind==='album'){setSelectedAlbum(entity);setPage('album')}else{setSelectedArtist(entity);setPage('artist')}}}/>})}</div></>}
    ...
    if(page==='search')return <><div className="search-hero"><Search/><input autoFocus value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>e.key==='Enter'&&rememberSearch()} onBlur={()=>rememberSearch()} placeholder={t('searchPlaceholder')}/>{query&&<IconButton onClick={()=>setQuery('')}><X/></IconButton>}</div>{!query&&recentSearches.length>0&&<div className="recent-searches"><div><b>{lang==='fa'?'جست‌وجوهای اخیر':'Recent searches'}</b><button onClick={async()=>{await api.clearRecentSearches();setRecentSearches([])}}>{t('clear')}</button></div><div>{recentSearches.map(item=><button key={item} onClick={()=>setQuery(item)}><Clock3 size={15}/>{item}</button>)}</div></div>}<SectionTitle title={query?t('search'):t('library')} subtitle={`${filtered.length} ${t('tracks')}`}/>{renderTracks(filtered)}</>
    ...
    if(page==='hidden')return <><SectionTitle title={t('hiddenSongs')} subtitle={`${hiddenSongs.length} ${t('tracks')}`}/><div className="track-table">{hiddenSongs.map((song,i)=><div className="track-row" key={song.id}><div className="track-index">{i+1}</div><div className="track-main"><Cover song={song} size={44}/><div className="ellipsis"><b>{song.title}</b><span>{song.artist||'—'}</span></div></div><div className="track-meta"><span>{song.album||'—'}</span></div><div className="track-actions"><button className="text-button" onClick={()=>unhideSong(song)}>{t('unhide')}</button></div><div className="track-duration">{fmt(song.duration)}</div></div>)}</div></>
    ...
    if(page==='album'&&selectedAlbum)return <><div className="entity-header"><Cover song={selectedAlbum.song} size={190}/><div><span>{t('album')}</span><h1>{selectedAlbum.name}</h1><p>{selectedAlbum.artist} · {selectedAlbum.songs.length} {t('tracks')} · {fmt(selectedAlbum.songs.reduce((a,s)=>a+s.duration,0))}</p><div className="header-actions"><button className="round-play" onClick={()=>playTrack(selectedAlbum.songs[0],selectedAlbum.songs)}><Play fill="currentColor"/></button><IconButton active={isPinned('album',selectedAlbum.name)} onClick={()=>togglePin('album',selectedAlbum.name)}><Pin/></IconButton></div></div></div>{renderTracks(selectedAlbum.songs)}</>
    if(page==='artist'&&selectedArtist)return <><div className="entity-header artist"><Cover song={selectedArtist.song} size={190}/><div><span>{t('artist')}</span><h1>{selectedArtist.name}</h1><p>{selectedArtist.songs.length} {t('tracks')}</p><div className="header-actions"><button className="round-play" onClick={()=>playTrack(selectedArtist.songs[0],selectedArtist.songs)}><Play fill="currentColor"/></button><IconButton active={isPinned('artist',selectedArtist.name)} onClick={()=>togglePin('artist',selectedArtist.name)}><Pin/></IconButton></div></div></div>{renderTracks(selectedArtist.songs)}</>
    ...
    if(page==='playlists')return <><SectionTitle title={t('playlists')} action={<div className="toolbar"><input className="playlist-search" value={playlistQuery} onChange={e=>setPlaylistQuery(e.target.value)} placeholder={t('search')}/><button onClick={()=>createPlaylist()}><Plus/>{t('newPlaylist')}</button><button onClick={()=>createPlaylistFolder()}><FolderPlus/>{t('newFolder')}</button></div>}/><div className="playlist-layout"><div className="folder-tree"><b>{t('playlistFolder')}</b>{playlistFolders.filter(f=>!f.hidden).map(f=><div className="playlist-folder" key={f.id} style={{paddingInlineStart:f.parent_id?24:8}}><Folder size={17}/><span>{f.name}</span><button title={lang==='fa'?'افکت پوشه':'Folder effects'} onClick={()=>setScopeEditor({kind:'folder',id:f.id,title:f.name,profile:f.audioProfile})}><AudioLines size={14}/></button><button onClick={()=>createPlaylist(f.id)}><Plus size={14}/></button><button onClick={()=>createPlaylistFolder(f.id)}><FolderPlus size={14}/></button></div>)}</div><div className="entity-grid">{playlists.filter(p=>p.name.toLowerCase().includes(playlistQuery.trim().toLowerCase())).map(p=><EntityCard key={p.id} title={p.name} subtitle={`${p.count} ${t('tracks')}`} icon={<ListMusic size={54}/>} badge={p.pinned?'PIN':''} onClick={()=>openPlaylist(p.id)}/>)}</div></div></>
    if(page==='playlist'&&selectedPlaylist){const visible=selectedPlaylist.songs.filter(s=>[s.title,s.artist,s.album].some(v=>String(v||'').toLowerCase().includes(playlistQuery.trim().toLowerCase())));const audioScope=selectedPlaylist.audioProfile?.enabled?selectedPlaylist.audioProfile:selectedPlaylist.folderAudioProfile?.enabled?selectedPlaylist.folderAudioProfile:null;return <><div className="entity-header playlist"><div className="playlist-big-icon"><ListMusic size={70}/></div><div><span>{t('playlists')}</span><h1>{selectedPlaylist.name}</h1><p>{selectedPlaylist.songs.length} {t('tracks')} · {fmt(selectedPlaylist.songs.reduce((a,s)=>a+s.duration,0))}</p><div className="header-actions"><button className="round-play" onClick={()=>playTrack(selectedPlaylist.songs[0],selectedPlaylist.songs,false,audioScope)}><Play fill="currentColor"/></button><IconButton active={!!selectedPlaylist.audioProfile?.enabled} title={lang==='fa'?'افکت پلی‌لیست':'Playlist effects'} onClick={()=>setScopeEditor({kind:'playlist',id:selectedPlaylist.id,title:selectedPlaylist.name,profile:selectedPlaylist.audioProfile})}><AudioLines/></IconButton><IconButton active={selectedPlaylist.pinned} onClick={async()=>{await api.patchPlaylist(selectedPlaylist.id,{pinned:!selectedPlaylist.pinned});setSelectedPlaylist(await api.playlist(selectedPlaylist.id));setPlaylists(await api.playlists())}}><Pin/></IconButton><IconButton onClick={async()=>{await api.deletePlaylist(selectedPlaylist.id);setPage('playlists');await refresh()}}><Trash2/></IconButton></div></div></div><div className="playlist-tools"><input value={playlistQuery} onChange={e=>setPlaylistQuery(e.target.value)} placeholder={t('search')}/><select value={selectedPlaylist.sortMode} onChange={async e=>{await api.patchPlaylist(selectedPlaylist.id,{sortMode:e.target.value});setSelectedPlaylist(await api.playlist(selectedPlaylist.id))}}><option value="custom">Custom</option><option value="title">{t('title')}</option><option value="artist">{t('artist')}</option><option value="album">{t('album')}</option><option value="added">{t('dateAdded')}</option><option value="duration">{t('duration')}</option></select><select value={selectedPlaylist.folderId||''} onChange={async e=>{await api.patchPlaylist(selectedPlaylist.id,{folderId:e.target.value?Number(e.target.value):null});setSelectedPlaylist(await api.playlist(selectedPlaylist.id));setPlaylists(await api.playlists())}}><option value="">{t('playlistFolder')} —</option>{playlistFolders.map(f=><option key={f.id} value={f.id}>{f.name}</option>)}</select></div>{renderTracks(visible,(s)=><div className="playlist-row-tools">{selectedPlaylist.sortMode==='custom'&&<><IconButton onClick={()=>reorderPlaylistSong(selectedPlaylist.songs.findIndex(x=>x.id===s.id),-1)}><ChevronUp size={15}/></IconButton><IconButton onClick={()=>reorderPlaylistSong(selectedPlaylist.songs.findIndex(x=>x.id===s.id),1)}><ChevronDown size={15}/></IconButton></>}<IconButton onClick={async()=>{await api.removePlaylistSong(selectedPlaylist.id,s.id);setSelectedPlaylist(await api.playlist(selectedPlaylist.id))}}><Trash2 size={16}/></IconButton></div>)},audioScope)</>}
    ...
    if(page==='create')return <div className="create-grid"><button onClick={()=>createPlaylist()}><ListMusic/><b>{t('newPlaylist')}</b><span>{t('fromLibrary')}</span></button><button onClick={()=>createPlaylistFolder()}><FolderPlus/><b>{t('newFolder')}</b><span>{t('nestedFolders')}</span></button><button onClick={()=>smart()}><Sparkles/><b>{t('smartMix')}</b><span>{t('recommendations')}</span></button><button onClick={addStream}><Globe2/><b>{lang==='fa'?'افزودن استریم':'Add stream'}</b><span>HTTP / HTTPS</span></button></div>
    if(page==='settings')return <SettingsPage t={t} lang={lang} settings={settings} saveSettings={saveSettings} installedFonts={installedFonts} folders={folders} chooseFolder={chooseFolder} chooseExcludedFolder={chooseExcludedFolder} removeExcludedFolder={removeExcludedFolder} scanFolders={scanFolders} scanSystem={scanSystem} busy={busy} stats={stats} cacheInfo={cacheInfo} clearCache={clearCache} exportBackup={exportBackup} importBackup={importBackup}/>
    return null
  }
  ...
  return <div className={`app-shell ${settings.reduceMotion?'reduce-motion':''}`}>
    ...
    {queueOpen&&<aside className="right-panel"><div className="panel-head"><div><h3>{t('queue')}</h3><small>{queue.length} {t('tracks')}</small></div><div><button className="text-button" onClick={()=>persistQueue([])}>{t('clear')}</button><IconButton onClick={()=>setQueueOpen(false)}><X/></IconButton></div></div>{queue.map((s,i)=><div className={`queue-item ${s.id===current?.id?'current':''}`} key={`${s.id}-${i}`}><Cover song={s} size={42}/><button onClick={()=>playTrack(s,queue)}><b>{s.title}</b><small>{s.artist}</small></button><div className="queue-order"><IconButton onClick={()=>moveQueue(i,-1)}><ChevronUp size={14}/></IconButton><IconButton onClick={()=>moveQueue(i,1)}><ChevronDown size={14}/></IconButton><IconButton onClick={()=>removeQueue(i)}><X size={14}/></IconButton></div></div>)}</aside>}
    ...
    {settingsOpen&&<Modal title={t('settings')} onClose={()=>setSettingsOpen(false)} wide><SettingsPage compact t={t} lang={lang} settings={settings} saveSettings={saveSettings} installedFonts={installedFonts} folders={folders} chooseFolder={chooseFolder} chooseExcludedFolder={chooseExcludedFolder} removeExcludedFolder={removeExcludedFolder} scanFolders={scanFolders} scanSystem={scanSystem} busy={busy} stats={stats} cacheInfo={cacheInfo} clearCache={clearCache} exportBackup={exportBackup} importBackup={importBackup}/></Modal>}
    ...
    {scopeEditor&&<AudioProfileEditor title={`${scopeEditor.title} · ${lang==='fa'?'افکت صوتی':'Audio effects'}`} t={t} profile={scopeEditor.profile} onClose={()=>setScopeEditor(null)} onSave={saveScopeProfile}/>}
    {toast&&<div className="toast">{toast}</div>}{busy&&<div className="busy-layer"><div className="spinner"/><span>{t('scanning')}</span></div>}
  </div>
}

function SettingsPage({t,lang,settings,saveSettings,installedFonts,folders,chooseFolder,chooseExcludedFolder,removeExcludedFolder,scanFolders,scanSystem,busy,stats,cacheInfo,clearCache,exportBackup,importBackup,compact=false}){
  ...
  return <div className={`settings-page ${compact?'compact':''}`}>
    ...
    <div className="theme-presets">{THEME_PRESETS.map(item=><button key={item.id} className={settings.themePreset===item.id?'selected':''} onClick={()=>saveSettings({themePreset:item.id})}><span className="theme-rank">{item.rank}</span><span className="theme-preview">{item.preview.map(color=><i key={color} style={{background:color}}/>)}</span><b>{item.name}</b><small>{item.description}</small></button>)}</div>
    <div className="settings-grid"><div className="setting-card span2"><label>{t('themeMode')}</label><div className="segmented">{[['system',Monitor],['dark',Moon],['light',Sun],['amoled',Disc3]].map(([id,Icon])=><button key={id} className={settings.themeMode===id?'active':''} onClick={()=>saveSettings({themeMode:id})}><Icon size={17}/>{t(id)}</button>)}</div></div>
      ...
      <div className="setting-card"><label>{t('font')}</label><select value={settings.fontFamily||'vazirmatn'} onChange={e=>saveSettings({fontFamily:e.target.value})}>{FONT_OPTIONS.map(f=><option key={f.id} value={f.id}>{f.name}</option>)}{!!installedFonts.length&&<option value="installed">{t('installedFont')}</option>}</select>{settings.fontFamily==='installed'&&<select value={settings.customFontName||''} onChange={e=>saveSettings({customFontName:e.target.value})}><option value="">{t('choose')}</option>{installedFonts.map(name=><option key={name} value={name}>{name}</option>)}</select>}</div>
      ...
      <div className="setting-card"><label>{lang==='fa'?'تراکم رابط':'Interface density'}</label><select value={settings.interfaceDensity||'comfortable'} onChange={e=>saveSettings({interfaceDensity:e.target.value})}><option value="compact">Compact</option><option value="comfortable">Comfortable</option><option value="spacious">Spacious</option></select></div><div className="setting-card"><Toggle checked={settings.reduceMotion} onChange={v=>saveSettings({reduceMotion:v})} label={t('reduceMotion')}/></div></div>
    <SectionTitle title={t('library')} subtitle={`${stats.songs||0} ${t('tracks')} · ${bytes(stats.bytes||0)}`}/><div className="settings-grid"><div className="setting-card"><label>{t('minDuration')}</label><div className="inline-input"><input type="number" min="0" max="3600" value={Math.round((settings.minDurationMs||0)/1000)} onChange={e=>saveSettings({minDurationMs:Number(e.target.value)*1000})}/><span>{t('seconds')}</span></div></div><div className="setting-card span2"><div className="button-row"><button onClick={chooseFolder}><FolderPlus/>{t('addFolder')}</button><button onClick={scanFolders} disabled={busy}><ScanSearch/>{t('scanFolders')}</button><button className="danger-outline" onClick={scanSystem} disabled={busy}><Gauge/>{t('scanSystem')}</button></div></div><div className="setting-card span3"><div className="folder-list mini">{folders.length?folders.map(f=><div key={f}><Folder/><span>{f}</span></div>):<small>—</small>}</div></div><div className="setting-card span3"><div className="panel-head"><b>{t('excludedFolders')}</b><button onClick={chooseExcludedFolder}><FolderPlus size={17}/>{t('excludeFolder')}</button></div><div className="folder-list mini">{(settings.excludedFolders||[]).length?(settings.excludedFolders||[]).map(f=><div key={f}><EyeOff/><span>{f}</span><IconButton onClick={()=>removeExcludedFolder(f)}><X size={16}/></IconButton></div>):<small>—</small>}</div></div><div className="setting-card span3"><b>{lang==='fa'?'فرمت‌های قابل اسکن':'Scanned audio formats'}</b><div className="format-picker">{FORMAT_OPTIONS.map(ext=>{const checked=(settings.audioExtensions||FORMAT_OPTIONS).includes(ext);return <label key={ext} className={checked?'active':''}><input type="checkbox" checked={checked} onChange={()=>saveSettings({audioExtensions:checked?(settings.audioExtensions||FORMAT_OPTIONS).filter(x=>x!==ext):[...(settings.audioExtensions||FORMAT_OPTIONS),ext]})}/>{ext.slice(1).toUpperCase()}</label>})}</div></div></div>
    <SectionTitle title={t('playback')}/><div className="settings-grid"><div className="setting-card"><label>{t('crossfade')} · {((settings.crossfadeMs||0)/1000).toFixed(1)}s</label><input type="range" min="0" max="12000" step="500" value={settings.crossfadeMs||0} onChange={e=>saveSettings({crossfadeMs:Number(e.target.value)})}/></div><div className="setting-card"><Toggle checked={settings.gaplessEnabled} onChange={v=>saveSettings({gaplessEnabled:v})} label={t('gapless')}/></div><div className="setting-card"><Toggle checked={settings.automixEnabled} onChange={v=>saveSettings({automixEnabled:v})} label={t('automix')}/></div><div className="setting-card"><Toggle checked={settings.advancedAutomixEnabled} onChange={v=>saveSettings({advancedAutomixEnabled:v})} label={t('advancedAutomix')}/></div><div className="setting-card"><Toggle checked={settings.rememberQueue} onChange={v=>saveSettings({rememberQueue:v})} label={t('rememberQueue')}/></div><div className="setting-card"><Toggle checked={settings.resumeLastSong} onChange={v=>saveSettings({resumeLastSong:v})} label={t('resumeLast')}/></div><div className="setting-card"><Toggle checked={settings.resumeOnAudioDeviceChange!==false} onChange={v=>saveSettings({resumeOnAudioDeviceChange:v})} label={lang==='fa'?'بازیابی پخش پس از تغییر هدفون/بلوتوث':'Recover after headphone/Bluetooth changes'} hint={lang==='fa'?'خروجی را ویندوز انتخاب می‌کند؛ NEO زمان پخش را حفظ و صدا را ادامه می‌دهد.':'Windows selects the output; NEO preserves position and resumes playback.'}/></div></div>
    ...
    <SectionTitle title={t('lyrics')}/><div className="settings-grid"><div className="setting-card"><Toggle checked={settings.translationEnabled} onChange={v=>saveSettings({translationEnabled:v})} label={t('translation')}/></div><div className="setting-card"><Toggle checked={settings.romanizationEnabled} onChange={v=>saveSettings({romanizationEnabled:v})} label={t('romanization')}/></div><div className="setting-card"><Toggle checked={settings.lyricsAutoScroll} onChange={v=>saveSettings({lyricsAutoScroll:v})} label={t('autoScroll')}/></div><div className="setting-card"><Toggle checked={settings.writeLyricsSidecar} onChange={v=>saveSettings({writeLyricsSidecar:v})} label={lang==='fa'?'ذخیره کنار آهنگ':'Write .lrc/.txt beside track'}/></div><div className="setting-card"><label>{t('lyricsSize')} · {settings.lyricsFontSize||20}</label><input type="range" min="12" max="42" value={settings.lyricsFontSize||20} onChange={e=>saveSettings({lyricsFontSize:Number(e.target.value)})}/></div></div>
    <SectionTitle title={t('offline')}/><div className="settings-grid"><div className="setting-card"><Toggle checked={settings.strictOfflineMode} onChange={v=>saveSettings({strictOfflineMode:v})} label={t('strictOffline')}/></div><div className="setting-card"><Toggle checked={settings.offlineBackupEnabled} onChange={v=>saveSettings({offlineBackupEnabled:v})} label={t('offlineBackup')}/></div><div className="setting-card"><label>{lang==='fa'?'حافظهٔ موقت':'Regeneratable cache'}</label><b>{bytes(cacheInfo?.bytes||0)} · {cacheInfo?.files||0} files</b><button onClick={clearCache}>{t('clear')}</button></div><div className="setting-card span2"><div className="button-row"><button onClick={exportBackup}><Download/>{t('exportBackup')}</button><button onClick={()=>fileRef.current?.click()}><Upload/>{t('importBackup')}</button><input ref={fileRef} hidden type="file" accept="application/json,.json" onChange={e=>importBackup(e.target.files?.[0])}/></div></div></div>
    <section className="about-card"><NeoLogo size={52}/><div><b>NEO Player</b><span>Version {packageInfo.version}</span><strong>Made by justawall</strong></div></section>
  </div>
}
...
function ProfileEditor({t,profile,onClose,onSave}){
  ...
  return <Modal title={t('trackAppearance')} onClose={onClose} wide><div className="profile-form"><label>{t('accent')}<input type="color" value={form.accent||'#ff7a1a'} onChange={e=>setForm({...form,accent:e.target.value})}/></label><label>Background<input type="color" value={form.background||'#101010'} onChange={e=>setForm({...form,background:e.target.value})}/></label><label>{t('visualizer')}<select value={form.visualizer||''} onChange={e=>setForm({...form,visualizer:e.target.value})}><option value="">{t('system')}</option><option value="bars">{t('bars')}</option><option value="waveform">{t('waveform')}</option><option value="pulse">{t('pulse')}</option></select></label><label>Wallpaper opacity<input type="range" min="0" max=".85" step=".05" value={form.wallpaperOpacity??.28} onChange={e=>setForm({...form,wallpaperOpacity:Number(e.target.value)})}/></label></div><AudioProfileFields form={{enabled:form.audioEnabled,eq:form.eq,bass:form.bass,virtualizer:form.virtualizer,loudness:form.loudness}} setForm={audio=>setForm({...form,audioEnabled:audio.enabled,eq:audio.eq,bass:audio.bass,virtualizer:audio.virtualizer,loudness:audio.loudness})} t={t}/><div className="modal-actions"><button onClick={onClose}>{t('cancel')}</button><button className="primary" onClick={()=>onSave(form)}>{t('save')}</button></div></Modal>
}

function AudioProfileFields({form,setForm,t}){
  const value={enabled:!!form.enabled,eq:[...(form.eq||[0,0,0,0,0])],bass:Number(form.bass||0),virtualizer:Number(form.virtualizer||0),loudness:Number(form.loudness||0)}
  const patch=(part)=>setForm({...value,...part})
  return <div className="audio-profile"><Toggle checked={value.enabled} onChange={enabled=>patch({enabled})} label={t('eq')} hint="Per-item audio profile"/><div className="eq-grid">{['60','230','910','3.6k','14k'].map((f,i)=><label key={f}><span>{f}</span><input disabled={!value.enabled} type="range" min="-12" max="12" step="1" value={value.eq[i]||0} onChange={e=>{const eq=[...value.eq];eq[i]=Number(e.target.value);patch({eq})}}/><b>{value.eq[i]||0}dB</b></label>)}</div><div className="profile-sliders"><label>{t('bass')} · {value.bass}%<input disabled={!value.enabled} type="range" min="0" max="100" value={value.bass} onChange={e=>patch({bass:Number(e.target.value)})}/></label><label>Virtualizer · {value.virtualizer}%<input disabled={!value.enabled} type="range" min="0" max="100" value={value.virtualizer} onChange={e=>patch({virtualizer:Number(e.target.value)})}/></label><label>Loudness · {value.loudness}dB<input disabled={!value.enabled} type="range" min="-12" max="12" value={value.loudness} onChange={e=>patch({loudness:Number(e.target.value)})}/></label></div></div>
}

function AudioProfileEditor({title,t,profile,onClose,onSave}){
  const [form,setForm]=useState(profile||{enabled:false,eq:[0,0,0,0,0],bass:0,virtualizer:0,loudness:0})
  return <Modal title={title} onClose={onClose} wide><AudioProfileFields form={form} setForm={setForm} t={t}/><div className="modal-actions"><button onClick={onClose}>{t('cancel')}</button><button className="primary" onClick={()=>onSave(form)}>{t('save')}</button></div></Modal>
}

...
class DesktopApiTests(unittest.TestCase):
    ...
    def test_full_backup_roundtrip_and_analysis(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            api=DesktopApi(store); data=api._full_backup_data(); self.assertEqual(3,data['version']); self.assertEqual('Demo.wav',Path(data['queuePaths'][0]).name); self.assertTrue(Path(data['queuePaths'][0]).is_file())
